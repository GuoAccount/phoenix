...
import com.g2s.phoenix.core.task.TaskEntryManager
...
/** 凤凰系统核心类 整合三元核心架构：调度器、执行引擎、韧性核心 */
class PhoenixCore
private constructor(
        private val configDir: File,
        private val context: android.content.Context? = null,
        // 创建一个私有的协程作用域，用于管理协程的生命周期
        private val scope: CoroutineScope =
                CoroutineScope(
                        Dispatchers.Default + SupervisorJob()
                ) // 使用默认调度器和SupervisorJob来确保子协程之间不会相互取消
) {

    companion object {
        @Volatile private var INSTANCE: PhoenixCore? = null
        ...
        fun getInstance(): PhoenixCore {
            return INSTANCE
                    ?: throw IllegalStateException("PhoenixCore未初始化，请先调用getInstance(configDir)")
        }
    ...
    suspend fun initialize(targetApp: String): Boolean {
        return initMutex.withLock {
            ...
            try {
                ...
                // 2. 加载或创建配置
                currentConfig =
                        configManager.loadConfig(ConfigSource.FILE)
                                ?: run {
    ...
    suspend fun submitTask(task: Task): Boolean {
        ...
        return try {
            ...
            if (success) {
                ...
                logError("任务提交失败", null, mapOf("taskId" to task.id, "reason" to "调度器拒绝"))
            }
            success
        } catch (e: Exception) {
        ...
        try {
            ...
            if (result.success) {
                ...
                if (result.exception != null) {
                    val exceptionInfo =
                            resilienceCore.classifyException(
                                    result.exception,
            ...
            val exceptionInfo = resilienceCore.classifyException(e, mapOf("taskId" to task.id))
            resilienceCore.handleException(exceptionInfo)
        }
    }
    ...
    private suspend fun updateStatus(status: PhoenixStatus) {
        ...
        if (oldStatus != status) {
            logInfo("系统状态变更", mapOf("oldStatus" to oldStatus.name, "newStatus" to status.name))
        }
    }
...
sealed class SystemEvent {
    ...
    data class StatusChange(val oldStatus: PhoenixStatus, val newStatus: PhoenixStatus) :
            SystemEvent()
}
